...
public class Character : SynchronizedObject
{
	...
	/// <summary>
	/// True if the character is crouching and the character belongs to ourselves (photonView.isMine is true)
	/// </summary>
	protected bool crouchActive;
	
	/// <summary>
	/// True if the player is walking upwards and the character belongs to ourselves (photonView.isMine is true).
	/// </summary>
	protected bool walkUpActive;	

	/// <summary>
	/// True if the player is walking downwards and the character belongs to ourselves (photonView.isMine is true).
	/// </summary>
	protected bool walkDownActive;	
	
	/// <summary>
	/// True if the player is walking left and the character belongs to ourselves (photonView.isMine is true)
	/// </summary>
	protected bool walkLeftActive;

	/// <summary>
	/// True if the player is walking right and the character belongs to ourselves (photonView.isMine is true)
	/// </summary>
	protected bool walkRightActive;
	
	/// <summary>
	/// True if the player is trying to jump and the character belongs to ourselves (photonView.isMine is true)
	/// </summary>
	...
	#region Photon Synchronization Variables
	
	/// <summary>
	/// Photon synchronization: The correct primary arm pivot.
	/// </summary>
	...
	protected override void Awake()
	{
		...
		// Default Photon serialization values
	...
	protected override void Update()
	{
		...
		if (IsDying)
		{
			...
			if (tryToUnspawn && photonView.isMine)
			{
	...
	protected void SetActiveHat(string prefab)
	{
		if (null != hatRenderer)
		{
			...
			if (!string.IsNullOrEmpty(prefab))
			{
				...
				hatRenderer.material.SetColor("_Emission", GetColor());
	...
	protected void SetActiveMouthpiece(string prefab)
	{
		if (null != mouthpieceRenderer)
			...
			if (!string.IsNullOrEmpty(prefab))
			{
				...
				mouthpieceRenderer.material.SetColor("_Emission", GetColor());
    ...
    protected override void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        if (stream.isWriting)
        {
			...
			stream.SendNext(primaryArmPivot.localPosition);
			stream.SendNext(primaryArmPivot.localRotation);
			stream.SendNext(CurrentPosture);
			stream.SendNext(facingDirection);
			stream.SendNext(ActiveWeaponName);
			stream.SendNext(ActiveHatName);
			stream.SendNext(ActiveMouthpieceName);
        }
        else
        {
            // Network player, receive data
			this.primaryArmPivot.localPosition = (Vector3)stream.ReceiveNext();
			this.correctPrimaryArmPivot = (Quaternion)stream.ReceiveNext();
			this.CurrentPosture = (Posture)stream.ReceiveNext();
			this.facingDirection = (FacingDirection)stream.ReceiveNext();
			string networkWeaponName = (string)stream.ReceiveNext();
			if (ActiveWeaponName != networkWeaponName)
			{
				SetActiveWeapon(networkWeaponName);
			}
			string networkHatName = (string)stream.ReceiveNext();
			if (ActiveHatName != networkHatName)
			{
				SetActiveHat(networkHatName);
			}
			string networkMouthpieceName = (string)stream.ReceiveNext();
			if (ActiveMouthpieceName != networkMouthpieceName)
			{
				SetActiveMouthpiece(networkMouthpieceName);
			}			
        }
    }
