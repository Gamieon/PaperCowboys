...
public class GameDirector : Photon.MonoBehaviour, IInputPoller
{	
	/// <summary>
	...
	/// The game version.
	/// </summary>
	public const string GameVersion = "tutorial.CHANGEME";
	...
	public static bool IsPlayingSolo {
		get {
			return (!IsOnline || 0 == PhotonNetwork.otherPlayers.Length);
		}
	}
	...
	/// </summary>
	public string PlayerCharacterPrefab = "PlayerCharacter";
	...
	/// <summary>
	/// Called by RPCSpawnYourself for you to spawn your player character. This can be
	/// called on any client.
	/// </summary>
	/// <param name='position'>
	/// Position.
	/// </param>
	void SpawnSelfCharacter(Vector3 position)
	{
		Debug.Log("Spawning player at " + position);		
		
		// Spawn yourself as the first player
		GameObject prefab = (GameObject)Resources.Load(PlayerCharacterPrefab);
		GameObject playerCharacter = (GameObject)PhotonNetwork.Instantiate(PlayerCharacterPrefab
			,position
	...
	/// </param>
	void RespawnDeadPlayer(PhotonPlayer player)
	{
		if (!IsGameOver)
		{
			...
			// Tell the player that they can respawn at the given X coordinate
			photonView.RPC("RPCSpawnYourself", player, x, y);
		}
	}
	...
	void Awake()
	{
		...
		// Special handling if we're offline
#if UNITY_EDITOR		
		if (!IsOnline)
		{
			Debug.Log("Playing offline");
			// Assign the player's photon name and custom propreties
			PlayerDirector.SetPhotonCustomPlayerProperties();
			PhotonNetwork.offlineMode = true;
			if (null == PhotonNetwork.room)
			{
				PhotonNetwork.CreateRoom(PhotonNetwork.playerName, false, false, 10);
			}
		}
#endif		
	...
	{	
		if (PhotonNetwork.isMasterClient)
		{
			...
			photonView.RPC("RPCSpawnYourself", PhotonTargets.All, startingPoint.transform.position.x, startingPoint.transform.position.y);
		}
	}
	...
	void Update() 
	{
		...
		if (timeUntilRespawn.HasValue && Time.time > timeUntilRespawn)
		{
			...
			photonView.RPC("RPCRespawnDeadPlayer", PhotonTargets.MasterClient, PhotonNetwork.player);
		}
	}	
	...
	public void PollGameOverStatus()
	{
		if (!IsGameOver)
		{
			...
			if (0 == playerCharacters.Count)
			{
				IsGameOver = true;
				Debug.Log("GAME OVER!");
				photonView.RPC("RPCGameOver", PhotonTargets.AllBuffered);
			}
		}
	}
	...
	void RenderChatBox()
	{
		...
		if (typingChatMessage.Length > 0 && Event.current.isKey)
		{
			if (Event.current.keyCode == KeyCode.Return || Event.current.keyCode == KeyCode.KeypadEnter)
			{
				...
				photonView.RPC("RPCChatMessage", PhotonTargets.All, PlayerDirector.Name + ": " + typingChatMessage, 
	...
	void RenderScores()
	{
		...
		foreach (PhotonPlayer player in PhotonNetwork.playerList)
		{
			...
			GUI.color = ColorDirector.H2RGB( (float)player.customProperties["Player.Hue"] );
			GUILayout.Label(player.name + ": " + (int)player.customProperties["Player.ActiveSession.Score"]);
			GUILayout.EndHorizontal();
		}
		GUILayout.EndArea();		
	}
	...
	/// <summary>
	/// Sent from the master client to any player to make them respawn
	/// </summary>
	/// <param name='x'>
	/// The X position where the player should respawn
	/// </param>
	[RPC]
	public void RPCSpawnYourself(float x, float y)
	{
		Debug.Log("RPCSpawnYourself: " + x + "," + y);
		if (null == selfCharacter)
		{
			SpawnSelfCharacter(new Vector3(x,y,ZPositions_Foreground));
		}
	}
	...
	/// </param>
	[RPC]
	public void RPCRespawnDeadPlayer(PhotonPlayer player)
	{
    ...
    public void OnPhotonPlayerConnected(PhotonPlayer player)
    {
		...
		if (PhotonNetwork.isMasterClient)
		{
			...
			photonView.RPC("RPCChatMessage", PhotonTargets.All, player.name + " has joined the game.", 
    ...
    public void OnPhotonPlayerDisconnected(PhotonPlayer player)
    {
		...
		{
			Color black = Color.black;
			photonView.RPC("RPCChatMessage", PhotonTargets.All, player.name + " has left the game.", 
				black.r, black.g, black.b);
		}		
    }
