...
public class SpawnPoint : Photon.MonoBehaviour 
{
	...
	void Update()
	{
		...
		if (PhotonNetwork.isMasterClient // We're the master client
			...
			)
		{
			...
			foreach (PlayerCharacter c in playerCharacters)
			{
					...
					c.transform.position.x < myTransform.position.x + xMax)
				{
					...
					PhotonNetwork.InstantiateSceneObject(spawnPrefab.ToString(), transform.position,
	...
	/// </param>
	[RPC]
	public void RPCSpawnedEnemyDied(PhotonView spawnedEnemy)
	{
    ...
    protected void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        if (stream.isWriting)
        {
			...
			stream.SendNext(totalSpawnedObjects);
			stream.SendNext(spawneeActive);
			stream.SendNext(timeEnemyDestroyed);
			stream.SendNext(spawnID);
        }
        else
        {
            // Network player, receive data
			this.totalSpawnedObjects = (int)stream.ReceiveNext();
			this.spawneeActive = (bool)stream.ReceiveNext();
			this.timeEnemyDestroyed = (float)stream.ReceiveNext();
