...
public sealed class Barrel : SynchronizedObject, IDamageTaker
{
	...
	protected override void Update()
	{
		...
		if (IsDying)
		{
			...
			if (tryToUnspawn && PhotonNetwork.isMasterClient)
			{
	...
	/// </param>
	[RPC]
	public void RPCBarrelTakeDamage(float damage, PhotonPlayer owningPlayer)
	{
		...
		{	
			...
			if (hitPoints - damage < MinHitpointsBeforeFrag)
			{
				...
				photonView.RPC("RPCBarrelBeginDying", PhotonTargets.All);
			}
		}
	}
    ...
    protected override void OnPhotonSerializeView(PhotonStream stream, PhotonMessageInfo info)
    {
        ...
        else
        {
			...
			this.hitPoints = (float)stream.ReceiveNext();
        }
    }
	...
	void IDamageTaker.TakeDamage(IDamageDealer damageDealer)
	{
		if (!IsDying)
		{
			...
			photonView.RPC("RPCBarrelTakeDamage", PhotonTargets.All, damage, (damageDealer.IsOwnedByPlayer() ? damageDealer.GetOwningPlayer() : null) );
		}
		else
		{
