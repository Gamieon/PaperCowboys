...
{		
		...
		if (PhotonNetwork.isMasterClient)
		{
				...
				Physics.Raycast(myTransform.position, new Vector3(-1,0,0), out hitInfo, 1.0f, (1 << (int)GameDirector.GameLayers.Enemy)) )
			{
				...
				if (null != enemyCharacter && !enemyCharacter.IsDying)
				{
					...
					photonView.RPC("RPCPlayerCharacterHorseBeginDying", PhotonTargets.All);
#endif
				}
			}		
		}
    ...
    {
        Debug.Log("OnPhotonInstantiate PLAYERCHARACTERHORSE " + info.sender);
	...
	void IDamageTaker.TakeDamage(IDamageDealer damageDealer)
	{
		if (!IsDying)
		{
			...
			PhotonNetwork.RPC(photonView, "RPCPlayerCharacterHorseTakeDamage", PhotonTargets.All, damage);
#endif
		}
		else
		{
