...
{		
	...
	void LateUpdate()
	{
		...
		if (networkView.isMine && !IsDying)
		{
			...
			if (myTransform.position.y < cameraTransform.localPosition.y - 6.0f) 
			{
				...
				networkView.RPC("RPCPlayerOnFootBeginDying", RPCMode.All);
			}
		}
	...
	/// </param>	
	[RPC]
    public void RPCNetworkInstantiate(NetworkPlayer owner)
	{
        Debug.Log("RPCNetworkInstantiate PLAYERONFOOT " + owner.ToString());
		gameObject.name = (string)NetworkDirector.GetCustomPlayerProperty(owner, "Player.Name");		
	}
	...
	void IDamageTaker.TakeDamage(IDamageDealer damageDealer)
	{
		if (!IsDying)
		{
			...
			networkView.RPC("RPCPlayerOnFootTakeDamage", RPCMode.All, damage);
#endif
		}
		else
		{
