...
public sealed class L3Boss : EnemyBoss, IDamageTaker
{
	...
	/// </param>
	[RPC]
	public void RPCBoss3TakeDamage(float damage, NetworkPlayer owningPlayer)
	{
		...
		{	
			...
			if (hitPoints - damage < MinHitpointsBeforeFrag)
			{
				...
				networkView.RPC("RPCBoss3BeginDying", RPCMode.All);
	...
	void IDamageTaker.TakeDamage(IDamageDealer damageDealer)
	{
		if (!IsDying)
		{
			...
			networkView.RPC("RPCBoss3TakeDamage", RPCMode.All, damage, damageDealer.GetOwningPlayer() );
		}
		else
		{
