...
public class SpawnPoint : MonoBehaviour 
{
			...
			)
		{
			...
			foreach (PlayerCharacter c in playerCharacters)
			{
					...
					c.transform.position.x < myTransform.position.x + xMax)
				{
					...
					NetworkDirector.InstantiateSceneObject(spawnPrefab.ToString(), transform.position,
	...
	/// </param>
	[RPC]
	public void RPCSpawnedEnemyDied(NetworkView spawnedEnemy)
	{
    ...
    protected void OnSerializeNetworkView(BitStream stream, NetworkMessageInfo info)
    {
        if (stream.isWriting)
        {
			...
			stream.Serialize(ref totalSpawnedObjects);
			stream.Serialize(ref spawneeActive);
			stream.Serialize(ref timeEnemyDestroyed);
			stream.Serialize(ref spawnID);
        }
        else
        {
