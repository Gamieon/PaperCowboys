...
public class GameDirector : MonoBehaviour, IInputPoller
{	
	/// <summary>
	...
	/// The game version.
	/// </summary>
	public const string GameVersion = "1.1native";
	...
	void SpawnSelfCharacter(Vector3 position)
	{
		...
		GameObject playerCharacter = (GameObject)NetworkDirector.Instantiate(PlayerCharacterPrefab
			,position
	...
	/// </param>
	void RespawnDeadPlayer(NetworkPlayer player)
	{
		if (!IsGameOver)
		{
			...
			networkView.RPC("RPCSpawnYourself", player, x, y);
		}
	}
	...
	void Awake()
	{
		...
		if (!IsOnline)
		{
			...
			// Assign the player's custom propreties
	...
	{	
		if (NetworkDirector.isMasterClient)
		{
			...
			networkView.RPC("RPCSpawnYourself", RPCMode.All, startingPoint.transform.position.x, startingPoint.transform.position.y);
		}
	}
	...
	void Update() 
	{
		...
		if (timeUntilRespawn.HasValue && Time.time > timeUntilRespawn)
		{
			...
			networkView.RPC("RPCRespawnDeadPlayer", NetworkDirector.MasterClient, Network.player);
		}
	}	
	...
	public void PollGameOverStatus()
	{
		if (!IsGameOver)
		{
			...
			if (0 == playerCharacters.Count)
			{
				IsGameOver = true;
				Debug.Log("GAME OVER!");
				networkView.RPC("RPCGameOver", RPCMode.AllBuffered);
			}
		}
	}
	...
	void RenderChatBox()
	{
		...
		if (typingChatMessage.Length > 0 && Event.current.isKey)
		{
			if (Event.current.keyCode == KeyCode.Return || Event.current.keyCode == KeyCode.KeypadEnter)
			{
				...
				networkView.RPC("RPCChatMessage", RPCMode.All, PlayerDirector.Name + ": " + typingChatMessage, 
	...
	void RenderScores()
	{
		...
		foreach (NetworkPlayer player in NetworkDirector.Players)
		{
			bool isAlive = false;
	...
	/// </param>
	[RPC]
	public void RPCRespawnDeadPlayer(NetworkPlayer player)
	{
	...
	void OnPlayerConnected(NetworkPlayer player)
	{
		...
		if (NetworkDirector.isMasterClient)
		{
			...
			networkView.RPC("RPCChatMessage", RPCMode.All, "A new player has joined the game.", 
	...
	void OnPlayerDisconnected(NetworkPlayer player)
	{
		...
		{
			Color black = Color.black;
			networkView.RPC("RPCChatMessage", RPCMode.All, "A player has left the game.", 
				black.r, black.g, black.b);
		}
		else
		{
