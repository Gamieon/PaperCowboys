...
public sealed class L3Boss : EnemyBoss, IDamageTaker
{
	/// <summary>
	...
	void FireGun()
	{
		if (NetworkDirector.isMasterClient && !IsDying)
		{
			...
			activeWeapon.BeginFiringPrimary(WeaponFiringDirection.LocalAxis, characterColor);
	...
	protected override void Start() 
	{
		...
		// Pursue the player
		InvokeRepeating("PlayFireSound", 0.5f, 2f);
		InvokeRepeating("FireGun", 0.5f, 2f);	
	}
	...
	/// </param>
	[RPC]
	public void RPCBoss3TakeDamage(float damage, uLink.NetworkPlayer owningPlayer)
	{
		...
		{	
			...
			if (hitPoints - damage < MinHitpointsBeforeFrag)
			{
				...
				if (uLink.Network.player == owningPlayer)
				{
				...
				{
					gameDirector.myNetworkView.RPC("RPCSetScore", owningPlayer, currentScore + pointValue);
				}

				// Tell everybody that this character is dying
				myNetworkView.RPC("RPCBoss3BeginDying", uLink.RPCMode.All);
				
				// Tell everybody that the boss has been defeated. Do it buffered in case someone
				// jumps in during the victory sequence.
				gameDirector.myNetworkView.RPC("RPCBossDefeated", uLink.RPCMode.AllBuffered);
			}
		}
	}
	...
	/// </summary>
	[RPC]
	public void RPCBoss3BeginDying()
	{
		if (!IsDying) 
		{
			BeginDying();
