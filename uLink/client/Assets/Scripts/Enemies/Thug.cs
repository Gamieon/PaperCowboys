...
public sealed class Thug : EnemyCharacter, IDamageTaker
{
	...
	/// </param>
	[RPC]
	public void RPCThugTakeDamage(float damage, uLink.NetworkPlayer owningPlayer)
	{
		...
		{	
			...
			if (hitPoints - damage < MinHitpointsBeforeFrag)
			{
				...
				if (Random.Range(0.0f, 1.0f) < 0.33f)
				{
					...
					gameDirector.myNetworkView.RPC("RPCGiveDrop", uLink.RPCMode.All, myTransform.position);					
				}
				
				// Tell everybody that this character is dying
				myNetworkView.RPC("RPCThugBeginDying", uLink.RPCMode.All);
			}
		}
	}
	...
	void IDamageTaker.TakeDamage(IDamageDealer damageDealer)
	{
		if (!IsDying)
		{
			...
			myNetworkView.RPC("RPCThugTakeDamage", uLink.RPCMode.All, damage, damageDealer.GetOwningPlayer() );
		}
		else
		{
