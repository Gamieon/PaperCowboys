...
public sealed class L1Boss : EnemyBoss, IDamageTaker
{
	...
	/// </param>
	[RPC]
	public void RPCBoss1TakeDamage(float damage, uLink.NetworkPlayer owningPlayer)
	{
		...
		{	
			...
			if (hitPoints - damage < MinHitpointsBeforeFrag)
			{
				...
				myNetworkView.RPC("RPCBoss1BeginDying", uLink.RPCMode.All);
	...
	void IDamageTaker.TakeDamage(IDamageDealer damageDealer)
	{
		if (!IsDying)
		{
			...
			myNetworkView.RPC("RPCBoss1TakeDamage", uLink.RPCMode.All, damage, damageDealer.GetOwningPlayer() );
		}
		else
		{
