...
/// </summary>
public class MainMenuDirector : uLink.MonoBehaviour 
{
	/// <summary>
	...
	void RenderGameSearchMenu()
	{
        ...
        else
        {
			...
			{
				uLink.HostData[] hosts = NetworkDirector.PollHostList();
				if (hosts.Length > 0)
				{
            ...
            foreach (uLink.HostData host in NetworkDirector.PollHostList())
            {
	...
	void uLink_OnServerInitialized() 
	{
		...
		Debug.Log("Populating self custom properties: key = " + uLink.Network.player.ToString());
